...
def get_cnns_features_from_dataloader(model, data_loader):
    model = model.to(device='cpu')
    deep_features_list = []
    label_list = []
    for data, label in data_loader:
        deep_feature = get_deep_features_from_cnns(model, data)
...
def get_deep_features_from_cnns(model, data):
    deep_feature = model(data)
...
class TsneVisualizationCNNs:
    ...
    def init(self):
        self.model.eval()
        deep_features, labels = get_cnns_features_from_dataloader(self.model, self.data_loader)
    ...
    def show_tsne(self, features, labels, num_classes):
        ...
        plt.title('TSNE visualization of out of %s layer' % 'the last linear', font)
