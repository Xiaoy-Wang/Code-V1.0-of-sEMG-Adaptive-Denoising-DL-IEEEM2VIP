...
def svd_denoise_signal(noisy_signal, svd_threshold_type='mutation_value'):
    ...
    if svd_threshold_type == 'mutation_value':
        # 滑动窗口结合差分法：使用窗口长度为2，步长为1的滑动窗截取S_values，计算均值，然后计算均值序列的差分（即斜率），取斜率最大的作为突变点。
    ...
    # 计算反对角线元素之和
    anti_diagonal_means = []
    num_anti_diagonals = min(X.shape)  # 反对角线的数量为行数和列数中较小的那个
...
class ICEEMDANPEDenoising:
    ...
    def fit(self, signal):
        ...
        # IMFs_pe = IMFs[0:self.pe_length, :]
        # K, pe_values = self.calculate_pe(IMFs_pe)
    ...
    def calculate_pe(self, signal):
        ...
        while True:
            ...
            # 计算重构后的序列的排列熵
            pe = permutation_entropy(reconstructed_imf, order=self.pe_order, delay=self.pe_tao, normalize=True)
            # 如果是第一次迭代或者排列熵增大，则继续迭代
