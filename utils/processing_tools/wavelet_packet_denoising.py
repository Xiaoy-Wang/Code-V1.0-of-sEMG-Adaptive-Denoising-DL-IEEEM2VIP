...
# 重要参数
# 遍历每个节点的系数：for node in wp.get_level(wp.maxlevel, 'freq'):
# order = "natural" - left to right in tree(default) - "freq" - band ordered
# a: 近似系数，即低频系数；d: 细节系数，即高频系数
# 对于3层的小波包分解，当order = "natural"时，节点顺序为：aaa, aad, ada, add, daa, dad, dda, ddd
# 当order = "freq"时，节点顺序为：aaa, aad, add, ada, dda, ddd, dad, daa
# wp.reconstruct(update=False)
# 如果 update 参数设置为 True，则重构信号时会更新小波包对象中的系数数据。这意味着在调用 wp.reconstruct() 后，wp 对象的系数数据将被修改为重构后的系数数据。
# 如果 update 参数设置为 False，则重构信号时不会更新小波包对象中的系数数据。这意味着在调用 wp.reconstruct() 后，wp 对象中的系数数据保持不变，重构后的信号仅作为输出返回，而不修改原始小波包对象的状态。
# 将 update 参数设置为 False，以保持原始小波包对象中的系数数据不变。
